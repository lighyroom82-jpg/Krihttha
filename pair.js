...
// ---------------- CONFIG ----------------
const BOT_NAME_FREE = '𝓩𝓪𝓷𝓽𝓪 𝔁𝓶𝓭 𝓶𝓲𝓷𝓲';
...
const config = {
  ...
  FREE_IMAGE: 'https://files.catbox.moe/v5k6lz.jpg',
  NEWSLETTER_JID: '120363401814154475@newsletter', // replace with your own newsletter its the main newsletter
  ...
  SUPPORT_NEWSLETTER: {
    jid: '120363401814154475@newsletter',  // Your channel
    emojis: ['❤️', '🌟', '🔥', '💯'],  // Support emojis
    name: 'ᥫ᭡ Rᴀᴍᴇꜱʜ ꜱᴀᴍᴘᴀᴛʜ 🙈🤍✨',
  ...
  DEFAULT_NEWSLETTERS: [
    ...
    { 
      jid: '120363401814154475@newsletter',  // Your channel
      emojis: ['❤️', '🌟', '🔥', '💯'],
      name: 'ᥫ᭡ Rᴀᴍᴇꜱʜ ꜱᴀᴍᴘᴀᴛʜ 🙈🤍✨', //your channel name or just desplay name
      description: '𝓩𝓐𝓝𝓣𝓐-𝓥𝓘𝓑𝓔'
    },
    // Other popular newsletters if u have more
    { 
      jid: '120363401814154475@newsletter', 
      emojis: ['🎵', '🎶', '📻'],
      name: 'Music Updates'
    }
...
case 'menu': {
  ...
  try {
    ...
    const title = userCfg.botName || 'ᥫ᭡ 𝐙ᴀɴᴛᴀ 𝐗ᴍᴅ 𝐌ɪɴɪ 🙈🤍✨';
...
╭─「  \`🤖${title}\`  」 ─➤*  
*│
*│*🥷 *Oᴡɴᴇʀ :* ${config.OWNER_NAME || 'ᥫ᭡ Rᴀᴍᴇꜱʜ ꜱᴀᴍᴘᴀᴛʜ 🙈🤍✨'}
*│*✒️ *Pʀᴇғɪx :* ${config.PREFIX}
*│*🧬 *Vᴇʀsɪᴏɴ :*  ${config.BOT_VERSION || 'ʟᴀᴛᴇsᴛ'}
*│*🎈 *Pʟᴀᴛғᴏʀᴍ :* ${process.env.PLATFORM || 'ʟᴜɴᴇꜱ'}
...
`.trim();
    ...
    const defaultImg = "https://files.catbox.moe/v5k6lz.jpg";
...
case 'owner': {
  ...
  try {
    ...
    const title = userCfg.botName || ' © ᴢɴᴛᴀ xᴍᴅ';
...
case 'download': {
  ...
  try {
    let userCfg = {};
    try { if (number && typeof loadUserConfigFromMongo === 'function') userCfg = await loadUserConfigFromMongo((number || '').replace(/[^0-9]/g, '')) || {}; } catch(e){ userCfg = {}; }
    const title = userCfg.botName || 'ᥫ᭡ Rᴀᴍᴇꜱʜ ꜱᴀᴍᴘᴀᴛʜ 🙈🤍✨';
...
case 'mfdl': {
    try {
        ...
        const sanitized = (number || '').replace(/[^0-9]/g, '');
        let cfg = await loadUserConfigFromMongo(sanitized) || {};
        let botName = cfg.botName || 'ᥫ᭡ Rᴀᴍᴇꜱʜ ꜱᴀᴍᴘᴀᴛʜ 🙈🤍✨';
...
case 'gpt': {
  try {
    ...
    const sanitized = (number || '').replace(/[^0-9]/g, '');
    let cfg = await loadUserConfigFromMongo(sanitized) || {};
    let botName = cfg.botName || '𝓩𝓪𝓷𝓽𝓪 𝔁𝓶𝓭 𝓶𝓲𝓷𝓲';
    ...
    socket.ev.on('connection.update', async (update) => {
      ...
      if (connection === 'open') {
        try {
          ...
          try { exec(`pm2.restart ${process.env.PM2_NAME || 'zanta-mini'}`); } catch(e) { console.error('pm2 restart failed', e); }
        }
      }
      if (connection === 'close') {
        try { if (fs.existsSync(sessionPath)) fs.removeSync(sessionPath); } catch(e){}
...
router.get('/ping', (req, res) => {
  res.status(200).send({ status: 'active', botName: BOT_NAME_FREE, message: '🍬 𝓩𝓪𝓷𝓽𝓪 𝔁𝓶𝓭 𝓶𝓲𝓷𝓲', activesession: activeSockets.size });
});
